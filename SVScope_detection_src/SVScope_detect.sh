...
# ========== Sort Algorithm1 & Algorithm2 using bcftools ==========
for i in 1 2; do
  VCF="Algorithm${i}.vcf"
  SORTED="Algorithm${i}_sorted.vcf"
  if [[ -f "$VCF" ]]; then
    echo "🔃 Sorting $VCF using bcftools ..."
...
# ========== Sort Algorithm3 using vcf-sort ==========
VCF3="Algorithm3.vcf"
SORTED3="Algorithm3_sorted.vcf"
if [[ -f "$VCF3" ]]; then
  echo "🔃 Sorting $VCF3 using vcf-sort ..."
  vcf-sort "$VCF3" > "$SORTED3"
  echo "✅ Sorted file generated: $SORTED3"
else
  echo "❌ Missing $VCF3. Skipping..."
fi
